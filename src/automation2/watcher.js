...
export function watchReadyFolder(config, onChange, options = {}) {
  const {
    debounceMs = 300,
  ...
  const trigger = () => {
    ...
    debounceTimer = setTimeout(() => {
      Promise.resolve(onChange()).catch((error) => {
        logger?.error?.("Automation 2 run triggered by the watcher failed", {
  ...
  const reportFatal = (error) => {
    const message =
      ...
      "New images copied into Instagram Ready will NOT get posting packages until the watcher is restarted.\n" +
      "To restart it, run: npm run automation2:watch";

    logger?.error?.("Folder watcher stopped permanently", { directory, error: describeError(error) });
    console.error(`\n[automation2] FATAL ${message}\n`);
  };
  ...
  function handleWatcherError(error) {
    ...
    retryCount += 1;
    console.error(
      `[automation2] ERROR Watcher lost connection to "${directory}" ` +
